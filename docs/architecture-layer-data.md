...
- データモデルでは、モデル定義を簡略化するため、`UserId` など、バリューオブジェクトに相当するモデルを定義しない
  - データモデル：データレイヤで一時的に使用するモデルのため、型の安全性よりも実装コストの削減を優先する
  - ドメインモデル：ドメインレイヤ、UI レイヤで使用し、バグ発生時のユーザーへの影響が大きいため、実装コストをかけて型の安全性を担保する
