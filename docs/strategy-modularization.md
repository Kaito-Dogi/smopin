...
| 観点             | 優先順位 | 説明                                                                 |
|:---------------|:-----|:-------------------------------------------------------------------|
| 再利用性           | ★★★  | ・AI エージェントを活用した迅速な開発の準備をする<br/>・本プロジェクトは習作であり、他プロジェクトに転用することを前提とする |
| シンプルさ          | ★★☆  | ・原則として、本当に必要だと判断してからモジュールを増やす<br/>                                 |
| その他マルチモジュールの利点 | ★☆☆  | 主に拡張性、テスト容易性、ビルド速度を向上する                                            |
...
AI エージェントの活用を前提として、次のデメリットを認める。

| 観点            | 理由                                    |
|:--------------|:--------------------------------------|
| ボイラープレートの増加   | AI エージェントを活用することで、コーディングコストが削減されるため   |
| 複雑性の増加・理解の難しさ | AI エージェントを活用することで、学習コストや認知コストが削減されるため |
...
| `data`               | ・データレイヤの抽象モジュール<br/>・Repository の実装、データモデル、DataSource のインターフェースを格納する               |
| `database:firestore` | ・データレイヤの具体実装モジュール<br/>・主に NetworkDataSource の実装を格納する<br/>・Firebase Firestore に依存する |
...
|:----------|:-----------------------------------------------------------|
| `app`     | アプリモジュール                                                   |
| `feature` | ・機能モジュール<br/>・画面単位ではなく、機能単位で分割する                           |
...
| `designSystem` | Color, Typography, Spacing などのカスタムテーマを格納する |
