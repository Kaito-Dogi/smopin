...
if [[ -n "$pr_arg" ]]; then
  pr_json=$(gh pr view "$pr_arg" --json number,url,headRefName,baseRefName,title,state) || pr_json=""
else
  pr_json=$(gh pr view --json number,url,headRefName,baseRefName,title,state) || pr_json=""
fi
