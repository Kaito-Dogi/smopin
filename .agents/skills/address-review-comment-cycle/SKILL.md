...
description: プルリクエストのレビューコメント対応サイクルを行う Skill。人間と AI エージェントのレビューコメントを取得し、指摘を修正または説明し、commit・push して、Gemini Code Assist などの AI エージェントへレビューを依頼または再依頼する。ユーザーが「レビューコメントに対応して」「Gemini Code Assist の指摘を直して」「人間のレビュー指摘も含めて修正して」「レビュー依頼と再レビュー依頼まで回して」などを依頼したときに使用する。
---
